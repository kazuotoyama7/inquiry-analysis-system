...
import { Search, Filter, BarChart3, Eye, ChevronLeft, ChevronRight, Copy, Loader2, Calendar, Download, PieChart } from 'lucide-react';
import './App.css';
...
function App() {
  ...
  const [currentView, setCurrentView] = useState('list'); // 'list' または 'analysis'
  ...
  function downloadCSV() {
    try {
      // 対象データを決定（選択されたものがあればそれを、なければ表示中のデータを使用）
      const targetData = selectedItems.size > 0 
